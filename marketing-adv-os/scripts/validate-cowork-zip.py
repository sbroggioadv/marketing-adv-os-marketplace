    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            for folder in sorted(skills_by_folder):
                ...
                # 4b.1 — REGRA HARD: skill folder so pode conter SKILL.md
                # Descoberto empiricamente em 2026-05-18: o backend do Cowork
                # rejeita silenciosamente plugins com arquivos extras dentro de
                # skill folders (ex: sizes.json, tokens.json, data.csv).
                # Plugin previdenciario funcional NUNCA tem arquivos auxiliares
                # dentro de pastas de skill. Auxiliares pertencem a
                # `templates/`, `data/` ou `scripts/` na raiz do plugin.
                for rel_in_folder in skills_by_folder[folder]:
                    if rel_in_folder != "SKILL.md":
                        rep.add("error", "SKILL_FOLDER_EXTRA_FILE",
                                f"skills/{folder}/{rel_in_folder}",
                                f"arquivo extra dentro de skill folder. Cowork rejeita "
                                f"silenciosamente. Mova para 'templates/', 'data/' ou "
                                f"'scripts/' na raiz do plugin e atualize referencias.")
