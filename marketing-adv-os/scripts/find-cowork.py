...
3. <CWD>/marketing/cowork-state.json (subindo ate 5 niveis)
4. <CWD>/.claude/settings.local.json campo env.COWORK_PATH
5. ~/.config/marketing-adv-os/active-cowork.json campo cowork_path
...
def _has_state(p: Path) -> bool:
    return (p / "marketing" / "cowork-state.json").exists()
...
def _try_active_cowork() -> Path | None:
    for cfg in (
        Path.home() / ".config" / "marketing-adv-os" / "active-cowork.json",
        Path.home() / ".marketing-adv-os" / "active-cowork.json",
    ):
        if cfg.exists():
            try:
