...
# Skills de outros plugins ja publicados pelo mesmo operador no Cowork —
# colidir com qualquer uma destas faz o backend rejeitar silenciosamente o upload.
PLUGIN_PAI_SKILLS = {
...
def main() -> int:
    ...
    extra_files_in_skill_folders: list[str] = []  # descoberta 2026-05-18

    # Check 4 (NOVO 2026-05-18): arquivos extras dentro de skill folders.
    # O backend do Cowork rejeita silenciosamente plugins com arquivos
    # alem de SKILL.md dentro de pastas de skill (ex: sizes.json, data.csv).
    # Auxiliares devem viver em templates/, data/ ou scripts/ na raiz do plugin.
    for skill_dir in sorted(SKILLS_DIR.iterdir()):
        if not skill_dir.is_dir() or skill_dir.name.startswith("_"):
            continue
        for f in skill_dir.iterdir():
            if f.is_file() and f.name != "SKILL.md":
                extra_files_in_skill_folders.append(f"{skill_dir.name}/{f.name}")
    ...
    print()
    print(f"[ARQUIVOS EXTRAS em skill folders]: {len(extra_files_in_skill_folders)}")
    for path in extra_files_in_skill_folders:
        print(f"  - skills/{path} (Cowork rejeita — mova para templates/data/scripts)")
    ...
    if extra_files_in_skill_folders:
        print()
        print(f"FALHA: {len(extra_files_in_skill_folders)} arquivo(s) extras em skill folders.")
        print("Cowork rejeita silenciosamente plugins com arquivos alem de SKILL.md")
        print("dentro de pastas de skill. Mova auxiliares para templates/, data/ ou scripts/")
        print("na raiz do plugin e atualize referencias no SKILL.md e nos scripts.")
        failed = True
    ...
    print()
    print(f"APROVADO: descriptions OK + sem colisoes + tamanhos OK + skill folders limpos.")
    return 0
