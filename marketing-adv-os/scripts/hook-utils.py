...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando marketing/ acima
- hook_state_load/save    → gerencia marketing/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/marketing/cowork-state.json."""
    start = start.resolve()
